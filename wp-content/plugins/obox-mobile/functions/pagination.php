<?php function mobile_pagination($container_class = "clearfix", $ul_class = "clearfix")
	{
		global $wp_query;
    ...
    <ul class="pagination">
        <?php if($currentpage !== ceil($numposts/get_option("posts_per_page"))) : ?>
            <li class="next"><?php next_posts_link("Siguiente"); ?></li>
        <?php endif;
        if($currentpage !== 1) : ?>
            <li class="previous"><?php previous_posts_link("Anterior"); ?></li>
        <?php endif; ?>
        <li class="page-count">Pagina <?php echo $currentpage?> de <?php echo $pagenum?></li>
    </ul>
<?php
		endif;
	} 
