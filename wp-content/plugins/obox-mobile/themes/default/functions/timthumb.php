...
class timthumb {
	...
	protected function processImageAndWriteToCache($localImage){
		...
		if ($new_width && !$new_height) {
			//New Width is set, but no New Height - calculate the new height
			$new_height = floor ($height * ($new_width / $width));
		} else if ($new_height && !$new_width) {
			//New Height is set, but no New Width - calculate the new width
			$new_width = floor ($width * ($new_height / $height));
		}
		else if ($new_width > $width && $new_height > $height) {
			//New Height & Width are larger than the original - keep the same size
			$new_width = $width;
			$new_height = $height;
		} else if ($new_width > $width && $new_height < $height) {
			//New Width is larger than the original, but the height is not - resize just the height
			$new_width = $width;
			$new_height = $new_height;
		} else if ($new_width < $width && $new_height > $height) {
			//New Height is larger than the original, but the width is not - resize just the width
			$new_width = $new_width;
			$new_height = $height;
		} else if ($width > $new_width && $height > $new_height) {
			//New Height & Width is smaller than the original - resize height & width
			$new_width = $new_width;
			$new_height = $new_height;
		} else if ($new_width > $width && !$new_height) {
			//New Width is larger than the original width, but the height is not set - don't resize anything
			$new_width = $width;
			$new_height = $height;
		}
