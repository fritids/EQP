	...
	public function get_avatar( $avatar = '', $id_or_email, $size = 96, $default = '', $alt = false ) {
		...
		if ( empty( $local_avatars[$size] ) ) {
			...
			$local_avatars[$size] = home_url( $local_avatars[$size] );
                ...
                $dividir = explode('/wp-content/', $local_avatars[$size]);
                $newURL = home_url().'/wp-content/'.$dividir[1];
                
		$author_class = is_author( $user_id ) ? ' current-author' : '' ;
		$avatar = "<img alt='" . esc_attr( $alt ) . "' src='" . $newURL . "' class='avatar avatar-{$size}{$author_class} photo' height='{$size}' width='{$size}' />";
...
//			echo 'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa';
//                        return;
