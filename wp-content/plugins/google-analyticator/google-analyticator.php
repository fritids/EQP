...
define('GOOGLE_ANALYTICATOR_CLIENTID', '937046899725-qipeihr7goh7mvj7kqp70kpahjrs1atu.apps.googleusercontent.com');
define('GOOGLE_ANALYTICATOR_CLIENTSECRET', '8qVq2bXz8ZclrQRQhru9PHlv'); //don't worry - this don't need to be secret in our case
...
function ajax_count() {
	...
	$yesterday = date('Y-m-d');
        ...
        //con permalink, segun url actual
        $plink = get_permalink($_POST['postid']);
        $site = site_url();
        $zone = explode($site, $plink);
        $page= $zone[1];
        $viewsAnalytics = $api->getMetrics('ga:pageviews', $before, $yesterday, 'ga:pagePath', false, 'ga:pagePath=='.$page, 1);
        $va = $viewsAnalytics->rows[0][1];
        if ($va==null) {$va = 0;}
        //con guid seg̼n url antigua        
        $guid = $wpdb->get_var("SELECT guid FROM $wpdb->posts WHERE ID = $pid LIMIT 1");
        $zoneguid = explode($site, $guid);
        $pageguid= urlencode(sanitize_url($zoneguid[1]));
        $viewsAnalyticsGid = $api->getMetrics('ga:pageviews', $before, $yesterday, 'ga:pagePath', false, 'ga:pagePath=='.$pageguid, 1);
        $vag = $viewsAnalyticsGid->rows[0][1];
        if ($vag==null) {$vag = 0;}
        //suma de ambas cosnultas (permalink y guid)        
        $vat = $va + $vag;
        
        $views = $vat!=0? $vat : (get_post_meta($_POST['postid'], '_visitas', true) + 1);
        ...
        $aryview = array("visitas" => $views, "vag" => $vag, "va" => $va);
...
add_action('wp_ajax_ajax_count', 'ajax_count');
add_action('wp_ajax_nopriv_ajax_count', 'ajax_count');
