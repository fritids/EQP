...
function oa_social_login_render_login_form ($source, $args = array())
{
	...
	if (!empty ($api_subdomain))
	{
		...
		//Themes
		$css_theme_uri_small = get_bloginfo("template_directory") . '/css/socialplugin.css';
		$css_theme_uri_default = get_bloginfo("template_directory") . '/css/socialplugin.css';
		...
		if (count ($providers) > 0)
		{
			...
			$output [] = '   "css_theme_uri": "' . get_bloginfo("template_directory") .'/css/socialLogin.css" ';
			$output [] = '  });';
			$output [] = ' </script>';
