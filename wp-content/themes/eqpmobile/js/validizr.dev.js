...
//        if( this.settings.delegateFields ){ this.$fields.on('change.validizr keyup.validizr', $.proxy( this.validateInput, this ) ); }
        if( this.settings.delegateFields ){ this.$form.on('change.validizr keyup.validizr', 'input:not([type="submit"]), select, textarea' , $.proxy( this.validateInput, this ) ); }
        if( this.settings.delegateBtn ){ this.$submitBtn.on('click.validizr', $.proxy( this.validateForm, this ) ); }
        this.$form.addClass('validizr-handled');
    };
    window.Validizr.prototype = {
        ...
        validateForm : function( event ){
            ...
            this.$form.find('input:not([type="submit"]), select, textarea').trigger('change.validizr');
            if( this.isFormValid() ){ 
