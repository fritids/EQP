...
function Swipe(container, options) {
  ...
  var events = {
    ...
    move: function(event) {
      ...
      // ie10 hack? 
      var touchCheck = window.navigator.msPointerEnabled ? false : event.touches.length > 1;
      if ( touchCheck || event.scale && event.scale !== 1) return;

      if (options.disableScroll) event.preventDefault();

      var touches = window.navigator.msPointerEnabled ? event : event.touches[0];
      ...
      if (!isScrolling) {
        ...
        options.onMove && options.onMove(index, delta.x, slides[index]);
      }
      ...
      if (!isScrolling) {
        ...
        if (isValidSlide && !isPastBounds) {
          ...
          options.edgeIndexCallback && options.edgeIndexCallback(index, slides[index]);
